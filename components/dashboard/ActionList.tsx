      ...
      <TabsContent value="boost" className="mt-3">
        <div className="space-y-2">
          {actions.boost.map((item) => (
            <div
              key={item.tweet_id}
              className="flex items-start gap-3 p-3 rounded-lg bg-slate-900 border border-slate-800 hover:border-slate-700 transition-colors"
            >
              ...
              <div className="text-right shrink-0">
                <p className="text-xs text-slate-400">{fmtNum(item.views)} 阅读</p>
                <p className="text-sm font-mono text-green-400 font-medium">{fmtEr(item.er)} ER</p>
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="space-y-2">
          {actions.kill.map((item) => (
            ...
            >
              ...
              <div className="text-right shrink-0">
                <p className="text-xs text-slate-400">{fmtNum(item.views)} 阅读</p>
                <p className="text-sm font-mono text-red-400 font-medium">{fmtEr(item.er)} ER</p>
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="space-y-2">
          {actions.reply.map((item) => (
            ...
            >
              ...
              <div className="text-right shrink-0">
                <p className="text-sm font-mono text-blue-400 font-medium">{item.replies} 回复</p>
              </div>
            </div>
          ))}
        </div>
